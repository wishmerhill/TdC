...
\paragraph{Coefficiente di sicurezza:} 
\begin{equation}
...
\section{Livello 0 - tensioni ammissibili}
dummy text

\chapter{Classificazione delle azioni}

\section{Combinazione dei carichi}

\section{Resistenze dei materiali}

\section{Carico da neve}

\section{Carico da vento}

\chapter{Materiali}

Per i materiali strutturali la norma prevede che si specifichi esattamente che materiale si usi.

Sono previste prove che ne attestino le caratteristiche, in particolare devono essere noti i legami costitutivi ($\sigma - \varepsilon$).

Deve essere effettuato il controllo di qualità del materiale messo in opera.

La responsabilità ricade sia sul progettista che sul direttore dei lavori.

\section{Conglomerato cementizio}

\section{Acciaio da calcestruzzo armato}

\chapter{Casi}

\section{Trave inflessa}

Si distinguono tre stadi in relazione alla freccia di una trave soggetta a flessione

\section{Elemento sottoposto a sforzo assiale (teso o compresso)}

\section{Pilastro cerchiato (sezione circolare)}

\section{Viscosità}

\section{Ritiro}

\chapter{Elemento semplicemente inflesso - calcolo elastico}
Ipotesi:
\begin{itemize}
\item La sezione ruota e trasla e si mantiene piana.
\item La sezione resistente è costituita dal calcestruzzo compresso (al di sopra dell'asse neutro quindi) e dalle armature (compresse e tese). Trascuro la resistenza a trazione del calcestruzzo. 
\item non vale la sovrapposizione degli effetti in quanto la sezione è parzializzata e trascurando la resistenza a trazione del CLS mi trovo con un diagramma delle tensioni che al di sotto dell'asse neutro ha senso solo per le armature inferiori in quanto il CLS non va considerato.
\end{itemize}
...
\subsection{Sezione semplicemente armata (calcolo elastico)}
...
\subsection{Sezione doppiamente armata (c. elastico)}

\section{Sezione a T}

\section{Flessione deviata (elemento a L)}

\section{Considerazione a margine (22/10)}
...
\section{Casi generali (tutti i tipo di CLS)}

\subsection{Campo 2}

\begin{framed}
\begin{align}
\text{per } & 0 < \varepsilon_c  \leq \varepsilon_{c_2} \nonumber \\[10pt] 
...
\begin{framed}
\begin{align}
\text{per: } & \varepsilon_{c_2} \leq \varepsilon_c \leq \varepsilon_{cu_2} \nonumber \\[10pt] 
...
\section{Casi di CLS con $C \leq C50/60$}

In questo caso specifico, vale $\varepsilon_{cu} = 3,5\tcperthousand - \varepsilon_{c2} = 2\tcperthousand - n = 2$.
